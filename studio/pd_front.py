...
def _pull_and_assemble(base, stamp, ids, tm, eng, th, verdict):
    ...
    PD_RDMA=os.environ.get("PD_RDMA","1")=="1"   # 9/17: whole capture dir over ONE MCDMA RDMA session (tar stream); HTTP per-file fallback
    rdma_dir_bin=os.path.expanduser("~/bin/rdma_pulldir.py"); tm["transport"]="tcp10"
    if PD_RDMA and os.path.exists(rdma_dir_bin):
        try:
            out=subprocess.run([sys.executable,rdma_dir_bin,PD_SPARK_SSH,f"{PD_SPARK_CAPDIR}/{stamp}",local],capture_output=True,text=True,timeout=PD_LONG_TIMEOUT)
            j=json.loads(out.stdout.strip().splitlines()[-1]) if out.stdout.strip() else {}
            if "bytes" in j:
                pulled=j["bytes"]; tm["transport"]="rdma"; tm["pull_files"]=j["files"]; tm["pull_wire_s"]=j["seconds_wire"]; tm["pull_wire_gbit"]=j["gbit_wire"]; tm["pull_total_s"]=j["seconds_total"]
                L(f"bridge: RDMA dir pull {j}")
            else: L(f"bridge: RDMA dir pull failed ({(out.stderr or out.stdout)[-300:]!r}) - http fallback")
        except Exception as ex: L(f"bridge: RDMA dir pull exception {ex!r} - http fallback")
    if tm["transport"]!="rdma":
...
def spark_prefill_stream(ids):
    ...
    # request-scoped streaming (hook PD_STREAM=auto): ask for a streaming capture for THE NEXT request, then POST.
    # A PD_STREAM=1 hook ignores the marker; a PD_STREAM=0 hook ignores it too and the start manifest below says so.
    try: urllib.request.urlopen(base+"/_stream_next", timeout=5).read(); tm["stream_marker"]=True
    except Exception as e: tm["stream_marker"]=False; L(f"bridge(stream): /_stream_next failed ({e!r}) — the hook decides by its own PD_STREAM")
    th=threading.Thread(target=_engine,daemon=True); th.start()
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # ring mode: the door only ingests (Sparks prefill -> RDMA -> blocks). If the caller asks, stop here and let the library decode.
        if self.headers.get("X-PD-Bridge-Only","0")=="1":
            body=json.dumps({"object":"pd.bridge","bridge":info}).encode()
            self.send_response(200); self.send_header("Content-Type","application/json"); self.send_header("X-PD-Bridge",json.dumps(info)); self.send_header("Content-Length",str(len(body))); self.end_headers(); self.wfile.write(body); return
